...
let MessageSource = {
  sendMessage:{
    remote(state){
      console.log('MessageSource.sendMessage');
  ...
  getMessages: {
    remote(state){
      console.log('MessageSource.getMessages');
      if(firebaseRef){
        firebaseRef.off();
      }
      firebaseRef = new Firebase('https://amber-heat-1598.firebaseio.com/messages/' + state.selectedChannel.key);
